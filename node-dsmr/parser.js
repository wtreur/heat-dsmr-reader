...
module.exports = telegram => {
  ...
  let additionalMeter = additionalMetersIndex[3];
    ...
    if (/^[^(]+\(/.test(telegram[index])) {
      switch (true) {
        ...
        case /0-[1-9]+:24.1.0/.test(telegram[index]):
          ...
          // Checking additional meter type (untested!)
          additionalMeter = additionalMetersIndex[values[1]];
          if (!output[additionalMeter]) {
            output[additionalMeter] = {}
          }
          continue
        ...
        case /0-[1-9]+:96.1.0/.test(telegram[index]):
          values = /^\d+-\d+:[^(]+\((.*)\)/.exec(telegram[index])
          if (values) {
            output[additionalMeter].equipmentId = Buffer.from(values[1], 'hex').toString()
          }
          continue
        ...
        case /0-[1-9]+:24.3.0/.test(telegram[index]):
          ...
          values = /^\d+-\d+:[^(]+\(([^)]+)\)/.exec(telegram[index])
          if (values) {
            output[additionalMeter].timestamp = calcTimestamp(values[[1]])
          }

          // Get usage value from next line
          values = /^\(([^)]+)/.exec(telegram[index + 1])
          if (values) {
            output[additionalMeter].totalConsumed = parseFloat(values[1])
            output[additionalMeter].reportedPeriod = 60
          }
          continue
        ...
        case /7-0:23.2.0/.test(telegram[index]):
          ...
          if (values) {
            ...
            output[additionalMeter].totalConsumed = parseFloat(values[2])
            output[additionalMeter].reportedPeriod = 1440
          }
          continue
        ...
        case /0-[1-9]+:24.4.0/.test(telegram[index]):
          values = /^\d+-\d+:[^(]+\((.*)\)/.exec(telegram[index])
          if (values) {
            output[additionalMeter].valvePosition = parseInt(values[1], 10)
          }
          continue
  ...
  for(let additionalMeter in additionalMetersIndex) {
    const currentMeter = additionalMetersIndex[additionalMeter]
    if (output[currentMeter]) {
      if (output[currentMeter].totalConsumed !== undefined && output.dsmrVersion < 5) {
        output[currentMeter].reportedPeriod = 60
      }

      if (output[currentMeter].totalConsumed !== undefined && output.dsmrVersion >= 5) {
        output[currentMeter].reportedPeriod = 5
      }
    }
  }
